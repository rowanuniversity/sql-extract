...
def test_run(cnxn, args):
    ...
    SqlExtractHandler(
        ...
        parsed.filename,
        parsed.outfile,
        in_delim=parsed.delimiter,
        in_quot=parsed.quotechar,
        in_positionals=parsed.positional_variables,
        in_named=parsed.bind_variables,
        in_text=parsed.text,
    )
...
class TestBase(unittest.TestCase):
    def test_basic_query(self):
        args = ["-t", "select 1 as column_1 from dual;", "-o", test_path]
        test_run(args)
    ...
    def test_column_name_case(self):
        args = ["-t", 'select 1 as "ColumnName" from dual;', "-o", test_path]
        test_run(args)
        with open(test_path, "r") as csv_file:
            self.assertTrue(next(csv.reader(csv_file)) == ["ColumnName"])

    def test_no_semicolon(self):
        args = ["-t", "select 1 as test_run from dual", "-o", test_path]
        valid = True
        try:
            test_run(args)
        except:
    ...
    def test_bind_params(self):
        args = [
            "-t",
            "select :0 as col_1, :1 as col_2, :2 as col_3 from dual;",
            "-o",
            test_path,
            "-b",
            "0=A",
