...
class SqlExtractHandler(object):
    ...
    def __init__(
        ...
        self.sql, self.text_input = self.configure_sql(in_sql_file, in_text)
        self.binds = self.configure_bind_vars(in_named, in_positionals)
        self.row_count = 0
        self.stdout = False
        if self.text_input:
            if in_out_file_location:
                self.csv_file = in_out_file_location
            else:
                self.csv_file = sys.stdout
                self.stdout = True
        else:
            self.csv_file = (
                in_out_file_location
                if in_out_file_location
                else os.path.splitext(in_sql_file)[0] + ".csv"
            )
        self.cnxn = conn
        ...
        csv.register_dialect(
            ...
            quotechar=self.dialect.quote_character,
    ...
    def write_results(self):
        ...
        """
        if self.stdout:
            csv_handler = self.csv_file
        else:
            csv_handler = open(self.csv_file, "w")
    ...
    def configure_sql(self, in_sql_file, in_text):
        ...
        if not (in_sql_file or in_text):
            raise IOError(
                "You must provide either a SQL file or text query to execute."
            )
        else:
        ...
        if in_binds:
            ...
            for param_input in in_binds:
                if not pattern.match(param_input):
                    raise IOError(
                        f"Invalid bind parameter format ({param_input}). Must be param=value."
                    )
                else:
                    parts = param_input.split("=")
                    out_binds[parts[0]] = "=".join(
                        parts[1:]
                    )  # in case there is an "=" actually in the value as well
        return out_binds
...
class CsvToXlsxHandler(object):
    ...
    def __init__(self, in_csv_file, out_xlsx_file=None):
        ...
        self.xlsx_file = (
            out_xlsx_file
            if out_xlsx_file
            else os.path.splitext(in_csv_file)[0] + ".xlsx"
        )
...
def get_sql_extract_argparser():
    ...
    p.add_argument(
        "filename",
        nargs="?",
        type=_validate_sql_file,
        help="SQL query to be exported as CSV",
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        "--login",
        type=str,
        default=os.environ.get("full_login"),
        help='Optional Oracle login string, defaults to look for an environment variable called "full_login".',
    )
    p.add_argument(
        "-p",
        "--password",
        ...
        help='Optional Oracle password, defaults to look for an environment variable called "db_password".',
    )
    p.add_argument(
        "-b",
        "--bind-variables",
        nargs="*",
        help="Optional named bind parameters. Enter as param1=value1 param2=value2",
    )
    p.add_argument(
        "-t",
        "--text",
        nargs="+",
        help="SQL query as text (instead of file). If a filename is specified, this argument will be ignored.",
    )
    p.add_argument(
        "positional_variables",
        nargs="*",
        help="Positional bind parameters to be passed to SQL file. If named bind variables (-b) are specified, this argument will be ignored.",
    )
    return p
...
def main(cmd_line_args):
    ...
    if not (cmd_line_args.login and cmd_line_args.password):
        sys.stderr.write(
            'Missing Oracle credentials. You must set "full_login" and "db_password". Alternatively, '
            "you can pass your credentials to this script with the --login and --password parameters."
        )
    else:
        conn = get_cx_oracle_connection(
            login=cmd_line_args.login, password=cmd_line_args.password
        )
        try:
            logging.basicConfig(level=logging.DEBUG)
            handler = SqlExtractHandler(
                conn,
                cmd_line_args.filename,
                cmd_line_args.outfile,
                in_delim=cmd_line_args.delimiter,
            ...
            in_display_path = (
                ...
                if cmd_line_args.filename
                else _read_input_sql(cmd_line_args.text)
            )
